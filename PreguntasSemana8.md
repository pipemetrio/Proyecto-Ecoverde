# Solución preguntas de Docker en Producción y Seguridad

## Tabla comparativa sobre el "desarrollo/local” y “producción"

| Aspecto                    | Desarrollo / Local                                                        | Producción                                                                           |
| -------------------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| Imagen base                | Puede ser una imagen completa para facilitar el desarrollo.               | Se utiliza una imagen mínima para reducir el tamaño y mejorar la seguridad.          |
| Usuario del proceso        | Generalmente se ejecuta como **root**.                                    | Se ejecuta con un usuario **no root** (por ejemplo, `appuser`).                      |
| Persistencia               | Puede no utilizar almacenamiento persistente.                             | Se emplean **volúmenes nombrados** para conservar los datos (`/data`).               |
| Secretos                   | Las credenciales suelen estar en variables de entorno o archivos locales. | Se administran mediante **Docker Secrets** (por ejemplo, `/run/secrets/banner_msg`). |
| Healthcheck                | Normalmente no se configura.                                              | Se utiliza **HEALTHCHECK** para comprobar que la aplicación funciona correctamente.  |
| Restart Policy             | El contenedor se inicia y detiene manualmente.                            | Se configura `restart: unless-stopped` para reiniciarlo automáticamente si falla.    |
| Puertos expuestos          | Se exponen los puertos necesarios para realizar pruebas.                  | Solo se publica el puerto estrictamente necesario (ejemplo: `8080`).                 |
| Filesystem de solo lectura | El sistema de archivos permite escritura.                                 | Se configura `read_only: true` para limitar la escritura y aumentar la seguridad.    |

## Explicación de: ¿Por qué un contenedor que funciona localmente no está listo automáticamente para producción?

Un contenedor que funciona correctamente en un entorno local no está listo automáticamente para producción porque, además de ejecutar la aplicación, debe ser seguro, estable y confiable. En producción se aplican medidas como usar imágenes mínimas, ejecutar el proceso con un usuario sin privilegios, proteger los secretos, limitar la escritura con un sistema de archivos de solo lectura, utilizar volúmenes para conservar datos, verificar el estado mediante healthcheck y configurar reinicios automáticos. Estas prácticas mejoran la seguridad, la disponibilidad y el mantenimiento de la aplicación.
